...
2. **git clone (url)** --> คัดลอกโปรเจคมาลงไว้ยังเครื่องตัวเอง (Local)
3. **git status** --> ตรวจสอยสถานะ ซึ่งมี 3 สถานะ
    * untracked files = ไฟล์ใหม่ที่ยังไม่ track
    * changes not staged = ไฟล์ที่แก้แล้วแต่ยังไม่ได้ Add
...
6. **git push** --> ส่ง commit จากเครื่องเราไป remote
...
* **cd ..** = ย้อน 1 ระดับ
* **cd ../..** = ย้อน 2 ระดับ
* **mkdir (name)** = สร้างโฟลเดอร์ขื่อตาม (name)
* **open** = เปิดโฟลเดอร์
